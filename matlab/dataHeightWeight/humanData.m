...
%% Initial Plots
figure(6)
plot(weight,height,'o'); hold on;
title('Height vs. Weight')
ylabel('Height (in)'); xlabel('Weight (kg)');
...
%% Use weight data to predict height (backslash operator)
figure(6);

%A*x=y or weight*x=height. To solve for x, we invert
A=[weight,ones(size(weight))]; %N by 2 matrix
y=[height]; %N by 1 vector
x=A\y; %this is the matrix inversion step. In this case we are calculating the 'pseudo-inverse'.
y_pred=polyval(x,weight);
plot(weight,y_pred)
legend('data','Linear Fit')